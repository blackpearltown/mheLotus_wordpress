...
(function($){
	...
	$("body").on('input click','.acf-fields,.acf-form-submit',function(a){$('.acff-message').remove()});
	...
	$('body').on('click','form.-submit input[type=submit]',function(e){
		...
		if(! $(this).hasClass('disabled')){
			$(this).addClass('disabled').after('<span class="acf-loading"></span>');
		...
		$.ajax({
			...
			success: function(response){
				...
				if( response.data.redirect ){
					...
					acf.unlockForm($form);	
					successMessage='<div class="acff-message"><div class="acf-notice -success acf-success-message"><p class="success-msg">'+response.data.success_message+'</p><span class="acff-dismiss close-msg acf-notice-dismiss acf-icon -cancel small"></span></div></div>';
