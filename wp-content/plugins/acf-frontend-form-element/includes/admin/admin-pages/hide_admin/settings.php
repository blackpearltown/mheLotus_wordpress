...
class ACFF_Hide_Admin_Settings{
		/**
	...
	*/
	public function redirect_non_admin_users() {
		...
		if ( is_admin() && ! current_user_can( 'manage_options' ) && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			$redirect_user = false;
			$hide_by_option = acf_decode_choices( get_option( 'acff_hide_by' ) );
			$login_redirect = get_option( 'acff_login_redirect' ); 
			...
			if( in_array( 'role', $hide_by_option ) ){
				...
				$redirect_roles = acf_decode_choices( get_option( 'acff_roles' ) ); 
	...
	public function get_settings_fields( $field_keys ){
		$local_fields = array(
			array(
				'key' => 'acff_hide_wp_dashboard',
				'label' => __( 'Hide WP Dashboard', 'acf-frontend-form-element' ),
				'name' => 'acff_hide_wp_dashboard',
				'type' => 'true_false',
				'instructions' => '',
				'required' => 0,
				'conditional_logic' => 0,
			...
			array(
				'key' => 'acff_login_redirect',
				'label' => __( 'Redirect to', 'acf-frontend-form-element' ),
				'name' => 'acff_login_redirect',
				'type' => 'url',
	...
	public function __construct() {
		...
		add_filter( 'acff/hide_admin_fields', [ $this, 'get_settings_fields'] );

	}
	
}
new ACFF_Hide_Admin_Settings( $this );	