		...
		if( $emails ){
			foreach( $emails as $email ){
				...
				foreach ( $fields as $key => $default ) {
					...
					$setting = acff()->dynamic_values->get_dynamic_values( $setting, $form );
					if ( ! empty( $setting ) ) {
						$fields[ $key ] = $setting;
					}
				}
				 ...
				 */
				$headers = apply_filters( 'acf_frontend/wp_mail_headers', $headers );
				 ...
				 */
				$fields['email_content'] = apply_filters( 'acf_frontend/wp_mail_message', $fields['email_content'] );
				 ...
				 */
				do_action( 'acf_frontend/mail_sent', $email, $form, $step );
			}
		}
			...
			switch ( $type ) {
				...
				case "credit":
					$meta = [
						...
						'value' => __( 'ACF Frontend', 'acf-frontend-form-element' ),
					];
					break;
			}			
...
acff()->remote_actions['email'] = new SendEmail();
