...
class ActionProduct extends ActionBase {
	...
	public function get_fields_display( $form_field, $local_field, $wg_id = '', $sub_fields = false, $saving = false ){
		...
		switch( $form_field['field_type'] ){
			...
			case 'grouped_products':
				$local_field['label'] =  __( 'Grouped Products', 'acf-frontend-form-element' );
				$local_field['type'] = 'product_grouped';
			break;
			case 'upsells':
				$local_field['label'] =  __( 'Upsells', 'acf-frontend-form-element' );
				$local_field['type'] = 'product_upsells';
			break;

			case 'cross_sells':
				$local_field['label'] =  __( 'Cross-sells', 'acf-frontend-form-element' );
...
/* 	public function create_product( $args ){	
	...
	public function save_form( $form ){	
		if( empty( $_POST['_acf_product'] ) ) return $form;

		if( empty( $_POST['acff']['woo_product'] ) && empty( $_POST['_acf_step'] ) ) return $form;

		$product_id = wp_kses( $_POST['_acf_product'], 'strip' );
		...
		if( ! empty( $_POST['acff']['woo_product'] ) ){
			foreach( $_POST['acff']['woo_product'] as $key => $value ){
				...
				if( ! in_array( $field_type, array_keys( $core_fields ) ) ){
					if( $field_type == 'product_types' ){
						$product_type = acf_extract_var( $_POST['acff']['woo_product'], $field['key'] );
						$pt_field = $field;
					}else{
				...
				$field_value = acf_extract_var( $_POST['acff']['woo_product'], $field['key'] );
				...
				$product_to_edit[ $core_fields[$field_type] ] = $field_value;
			}
		}
		...
		if( $product_to_edit['ID'] == 0 ){
			...
			update_metadata( 'post', $product_id, 'acff_form_source', $wg_id );
		}else{
			wp_update_post( $product_to_edit );
			update_metadata( 'post', $product_id, 'acff_form_edited', $wg_id );
		}
		...
		if( ! empty( $metas ) ){
			foreach( $metas as $meta ){
				acf_update_value( $meta['value'], $product_id, $meta['field'] );
			}
		}
