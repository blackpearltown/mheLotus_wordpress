			...
			foreach( $settings['emails_to_send'] as $email ){
				...
				if( $send_email ){
					...
					$fields = [
						...
						'email_reply_to' => 'noreplay@' . acf_frontend_get_site_domain(),
						'email_reply_to_name' => '',
					...
					foreach ( $fields as $key => $default ) {
						...
						$setting = acff_shortcode( $setting, $post_id );
						if ( ! empty( $setting ) ) {
							$fields[ $key ] = $setting;
						}
					}
					...
					$fields['email_content'] = apply_filters( 'acff/wp_mail_message', $fields['email_content'] );
					...
					do_action( 'acff/mail_sent', $settings, $email, $fields, $step );
				}
			}
		}
